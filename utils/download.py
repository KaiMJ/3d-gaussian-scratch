...
def hf_download_path(repo_path: str, odir: str, max_try: int = 5):
    ...
    while True:
        ...
        try:
            api.hf_hub_download(repo_id=repo_root, filename=rel_path,
                                repo_type='dataset', local_dir=odir, cache_dir=join(odir, '.cache'))
            return True
...
def download_benchmark(args):
    ...
    filepath_dict = pickle.load(
        open(join(output_dir, '.cache/filelist.bin'), 'rb'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--odir', type=str,
                        help='output directory', default='DL3DV-10K-Benchmark')
    parser.add_argument('--subset', choices=['full', 'hash'],
    ...
    parser.add_argument(
        '--hash', type=str, help='If set subset=hash, this is the hash code of the scene to download', default='')
